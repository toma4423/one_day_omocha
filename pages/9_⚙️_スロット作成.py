...
symbols_str = st.text_input(
    "図柄のリスト", 
    ", ".join(st.session_state.slot_config_edit["symbols"]),
    key="symbols_edit_input"
)

# --- 役の編集 ---
st.write("---")
st.subheader("💰 役と配当の設定")
st.write("各役の名称、図柄パターン、スコアを設定します。パターンのどこかに `ANY` を入れると、そのマスは何の図柄でも成立します。")
